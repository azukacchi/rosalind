...
# plus any new offspring. A key observation is that the number of offspring in any month is equal to the number of rabbits that were alive two months prior.
...
with open(os.path.join(BASE_DIR, 'rosalind_fib.txt')) as f:
    a = f.read()
# n = 6
# k = 1
...
def fib(n,k):
    prev_2_month, prev_1_month = 1,1
    for i in range(2,n):
        # print('month ',i+1)
        # print(prev_2_month,prev_1_month)
        prev_2_month, prev_1_month = prev_1_month, k*prev_2_month + prev_1_month
        # print(prev_1_month)
    print(prev_1_month)
